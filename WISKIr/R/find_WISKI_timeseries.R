#' Find all timeseries on WISKI web server for a specified station
#' @description Queries a WISKI web server to get a list of available time series which have the specified station name.
#' @param stationName Required. A character vector containing the name of the station. May contain wildcard characters.
#' @param site.url Optional. Optional. A character vector containing the url of the WISKI web server. Defaults to the Global Institute for Water Security (GIWS) server: "http://giws.usask.ca:8080/
#' @details As this packages is intended for use by the Saskatoon hydrological community, it is usually unnecessary to specify the web server.
#' @return A returned data frame containing \item{station_name}{Name of station, specified by stationName} \item{station_no}{WISKI number of station} \item{station_id}{WISKI station ID number} \item{ts_id}{WISKI time series ID number} \item{ts_name}{WISKI time series name} \item{parametertype_id}{WISKI ID number of parameter type of time series} \item{parametertype_name}{WISKI name of parameter type of time series}
#' @author Kevin Shook
#' @seealso \code{\link{get_WISKI_timeseries_metadata}} \code{\link{get_WISKI_timeseries_values}}
#' @examples
# find all of the time series for Fisera Ridge
#' WISKI.timeseries <- find_WISKI_timeseries('Fisera*')
#' print(WISKI.timeseries)
#' @export

find_WISKI_timeseries <- function(stationName, site.url='http://giws.usask.ca:8080/'){
  # finds all time series for a given station - station name can include wild cards
  # returns data frame containing all available time series
  stock <-'KiWIS/KiWIS?service=kisters&type=queryServices&request=getTimeseriesList&datasource=0&format=ascii&station_name='
  
  # find spaces and replace with '%20'
  stationName <- str_replace(stationName, ' ', '%20')
  
  WISKIstring <- paste(site.url, stock, stationName, sep='')
  data <- readLines(WISKIstring)
  # convert to data frame
  # split header on tab char
  data.split <- strsplit(data, '\t')
  data.dataframe <- as.data.frame(do.call('rbind',data.split), stringsAsFactors = FALSE)
  variable.names <- data.dataframe[1,] 
  timeseries.dataframe <- data.dataframe[2:nrow(data.dataframe),]
  names(timeseries.dataframe) <- variable.names
  return(timeseries.dataframe)
}
